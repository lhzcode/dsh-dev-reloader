...
import type { SettingsEditOp, SettingsTransportSnapshot } from './settings-transport.js';
export type CardPhase = SupervisorPhase | 'unknown';
export type DevReloaderCardState = SettingsTransportSnapshot<SupervisorConfig>;
export type CommandType = 'get-status' | 'update-config' | 'rebuild' | 'restart' | 'pause' | 'stop';
...
export interface SettingsCardFace {
    ...
    readonly command: (type: CommandType, options?: CommandOptions) => Promise<CommandResult>;
