...
export function protocolError(message: string, cause?: unknown): IpcError {
  return new IpcError('IPC_PROTOCOL_ERROR', message, errorOptions(cause))
}

export function disconnected(
  message = 'IPC peer disconnected',
  cause?: unknown,
): IpcError {
...
function encodeFrame(value: WireEnvelope): Buffer {
  let encoded: Buffer
  try {
...
class BoundedSocketWriter {
  ...
  private failed: IpcError | undefined

  constructor(private readonly socket: Socket) {
    socket.once('close', () => this.abort(disconnected()))
  }

  write(value: WireEnvelope): Promise<void> {
    ...
    if (
      frameCount + 1 > MAX_OUTBOUND_FRAMES
  ...
  private abort(error: IpcError): void {
  ...
  private pump(): void {
    if (this.failed !== undefined || this.active !== undefined) return
...
const writers = new WeakMap<Socket, BoundedSocketWriter>()

function socketWriter(socket: Socket): BoundedSocketWriter {
...
export function waitForSocketClose(socket: Socket): Promise<void> {
 ...
 */
export function privateListenOptions(
  endpoint: string,
...
export async function verifyUnixEndpoint(endpoint: string): Promise<void> {
...
export function createInboundQueue(): InboundQueueState {
  return {
    frames: [],
    bytes: 0,
    processing: false,
...
export function failInboundQueue(state: InboundQueueState): void {
  state.failed = true
  state.frames.length = 0
  state.bytes = 0
}

function resumeQueueIfReady(socket: Socket, state: InboundQueueState): void {
  if (
    !state.failed
    && !state.processing
    && !socket.destroyed
...
export function enqueueInboundFrames(
  socket: Socket,
  state: InboundQueueState,
  decoder: ByteLineDecoder,
  chunk: Buffer,
  maxFrames: number,
  consume: (frame: Buffer) => Promise<void>,
  ...
  if (
    state.frames.length + frames.length > maxFrames
