...
import {
  ...
  MAX_QUEUED_FRAMES,
  closeSocket,
  createInboundQueue as createQueueState,
  decodeEnvelope,
  disconnected,
  enqueueInboundFrames as enqueue,
  failInboundQueue as failQueue,
...
export interface ListenForBridgesOptions {
  ...
  readonly onCommand?: (
    ...
    peer: IpcPeer,
  ) => IpcCommandOutcome | void | Promise<IpcCommandOutcome | void>
}
...
function validateToken(token: string): void {
  if (!/^[a-f0-9]{64}$/.test(token)) {
    throw new TypeError(
      'IPC token must be exactly 32 random bytes encoded as lowercase hex',
    )
  }
}
...
function isSupervisorCommand(envelope: WireEnvelope): boolean {
  return SUPERVISOR_COMMAND_TYPES.has(envelope.type)
}

export async function listenForBridges(
  options: ListenForBridgesOptions,
): Promise<IpcServer> {
  if (!options.endpoint) {
  ...
  const peers = new Set<ServerPeerState>()
  let closed = false
  ...
  const server: Server = createServer(socket => {
    ...
    const peerState: ServerPeerState = {
      socket,
      authenticated: false,
      hello: undefined,
      failed: false,
    ...
    let handshakePhase:
      | 'awaiting-init'
      | 'awaiting-hello'
      | 'authenticated' = 'awaiting-init'
    let authenticatedClientNonce: string | undefined
    ...
    const consume = async (frame: Buffer): Promise<void> => {
      ...
      if (handshakePhase === 'awaiting-init') {
        requireExactEnvelope(
          envelope,
          'authentication-init',
          ['clientNonce'],
        )
        if (
          typeof envelope.clientNonce !== 'string'
        ...
        await writeFrame(socket, {
          ...
          serverProof: createProof(
            options.token,
            SERVER_DOMAIN,
            serverNonce,
            authenticatedClientNonce,
          ),
        })
        return
      }
      ...
      if (handshakePhase === 'awaiting-hello') {
        ...
        try {
          ...
          throw new IpcError(
            'IPC_AUTHENTICATION_FAILED',
            'invalid bridge hello',
            { cause: error },
          )
        }

        if (
          hello.clientNonce !== authenticatedClientNonce
          || !digestEqual(
            hello.clientProof,
            createProof(
              options.token,
        ...
        if (!await options.validateHost(hello)) {
          throw new IpcError(
            'IPC_AUTHENTICATION_FAILED',
            'bridge host validation failed',
          )
        }
        if (peerState.failed || socket.destroyed) return
      ...
      if (isSupervisorCommand(envelope)) {
        ...
        if (inFlight.has(command.requestId) || recent.has(command.requestId)) {
          throw new IpcError(
            ...
            `duplicate IPC request id: ${command.requestId}`,
          )
        }
        ...
        void Promise.resolve()
          ...
          .then(outcome => {
            if (peerState.failed) return
            const result: SupervisorEvent = outcome?.error === undefined
              ? {
                  protocolVersion: PROTOCOL_VERSION,
                  type: 'command-result',
    ...
    socket.on('data', chunk => enqueue(
      socket,
      queue,
      decoder,
      chunk,
      peerState.authenticated ? MAX_QUEUED_FRAMES : 1,
      consume,
      fail,
    ))
  })
  ...
  return {
    ...
    async broadcast(event): Promise<void> {
      decodeSupervisorEvent(event)
      ...
      await Promise.all(
        livePeers.map(peer => writeFrame(peer.socket, event)),
      )
    },
    close(): Promise<void> {
...
interface PendingRequest {
  readonly resolve: (result: IpcCommandResult) => void
  readonly reject: (error: IpcError) => void
}

export async function connectToSupervisor(
  options: ConnectToSupervisorOptions,
): Promise<IpcClient> {
  if (!options.endpoint) {
    throw new TypeError('IPC endpoint is required')
  }
  ...
  let settleAuthentication!: (error?: IpcError) => void
  ...
  const rejectPending = (error: IpcError): void => {
  ...
  socket.once('close', () => {
    ...
    const error = authenticated
      ...
      : new IpcError(
          'IPC_AUTHENTICATION_FAILED',
          'IPC authentication did not complete',
        )
    if (!authenticated) {
      settleAuthentication(error)
    }
  ...
  let handshakePhase:
    ...
    | 'awaiting-proof'
    | 'awaiting-result'
    | 'authenticated' = 'awaiting-challenge'
  let authenticatedServerNonce: string | undefined
  ...
  const consume = async (frame: Buffer): Promise<void> => {
    ...
    if (handshakePhase === 'awaiting-challenge') {
      ...
      if (
        typeof envelope.serverNonce !== 'string'
    ...
    if (handshakePhase === 'awaiting-proof') {
      ...
      if (
        ...
        || !digestEqual(
          envelope.serverProof,
          createProof(
            options.token,
            SERVER_DOMAIN,
            authenticatedServerNonce,
            clientNonce,
          ),
        )
      ) {
        throw new IpcError(
          'IPC_AUTHENTICATION_FAILED',
          'invalid server authentication proof',
        )
      }
      const helloCandidate: BridgeHello = {
        protocolVersion: PROTOCOL_VERSION,
        type: 'bridge-hello',
        ...options.hello,
        clientNonce,
        clientProof: createProof(
          options.token,
          CLIENT_DOMAIN,
          authenticatedServerNonce,
          clientNonce,
    ...
    if (handshakePhase === 'awaiting-result') {
      ...
      if (
        ...
        throw new IpcError(
          'IPC_AUTHENTICATION_FAILED',
          'invalid final authentication proof',
        )
      }
      authenticated = true
      handshakePhase = 'authenticated'
    ...
    if (event.type === 'command-result') {
      ...
      if (request === undefined) {
        throw protocolError(
          `unknown command result request id: ${event.requestId}`,
        )
      }
  ...
  return {
    ...
    emit(event): Promise<void> {
      if (closed || closing || failed) return Promise.reject(disconnected())
    ...
    request(command): Promise<IpcCommandResult> {
      ...
      return new Promise<IpcCommandResult>((resolve, reject) => {
