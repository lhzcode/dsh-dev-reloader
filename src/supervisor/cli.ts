...
import { DEFAULT_SUPERVISOR_CONFIG, type SupervisorConfig } from '../shared/config.js'
...
import {
  ...
  type HandoffChannelHandle,
  type HandoffSnapshot,
} from './handoff.js'
import {
  listenForBridges,
  type IpcServer,
  type ListenForBridgesOptions,
} from './ipc.js'
import { acquireSupervisorLock, type LockLease } from './lock.js'
...
export type SupervisorCliArguments =
  | { readonly mode: 'serve'; readonly profile: string }
  | { readonly mode: 'handoff'; readonly profile: string }

export interface SupervisorCliAdapters {
  readonly serve: (
    arguments_: Extract<SupervisorCliArguments, { mode: 'serve' }>,
  ) => void | Promise<void>
  readonly handoff: (
    arguments_: Extract<SupervisorCliArguments, { mode: 'handoff' }>,
  ) => void | Promise<void>
}
...
export function parseCliArguments(argv: readonly string[]): SupervisorCliArguments {
  let mode: SupervisorCliArguments['mode'] | undefined
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--serve' || argument === '--handoff') {
      ...
      if (mode !== undefined) throw new Error('exactly one supervisor mode is required')
      mode = nextMode
      continue
    }
...
async function readSupervisorToken(paths: RuntimePaths): Promise<string | undefined> {
  let handle
  try {
...
export async function loadOrCreateSupervisorToken(paths: RuntimePaths): Promise<string> {
...
function defaultInstallSignalHandlers(handler: () => void): () => void {
...
function defaultWatchHostExit(pid: number, handler: () => void): () => void {
  let closed = false
...
export function createDefaultSupervisor(context: SupervisorFactoryContext): DevReloaderSupervisor {
  ...
  const createLifecycle = (nextConfig: SupervisorConfig, seedLaunch?: HostLaunchSpec) => {
...
function defaultRuntime(): SupervisorCliRuntime {
...
function spawnStandbyProcess(options: { profile: string; transactionId: string }): { readonly pid: number } {
  const child = spawn(
    process.execPath,
    [supervisorCliEntry(), '--handoff', '--profile', options.profile],
    {
...
function supervisorCliEntry(): string {
...
function resolveSupervisorCliEntry(): string {
  // Running from source (tsx/ts-node) yields cli.ts; the built artifact cli.js.
  return fileURLToPath(import.meta.url)
}

function sameLaunchIdentity(left: HostLaunchSpec, right: HostLaunchSpec): boolean {
...
export async function serveSupervisor(
  arguments_: Extract<SupervisorCliArguments, { mode: 'serve' }>,
  runtime: SupervisorCliRuntime,
  seed?: { readonly lease?: LockLease; readonly launch: HostLaunchSpec; readonly config: SupervisorConfig },
): Promise<void> {
  const paths = await runtime.resolvePaths(arguments_.profile)
  let lease: LockLease | undefined = seed?.lease
  let supervisor: DevReloaderSupervisor | undefined
  let server: IpcServer | undefined
  let removeSignalHandlers: (() => void) | undefined
  let removeHostExitWatcher: (() => void) | undefined
  let bridgeBootId: string | undefined
  let activeLaunch: HostLaunchSpec | undefined = seed?.launch
  let helloTail: Promise<void> = Promise.resolve()
  ...
  try {
    if (lease === undefined) lease = await runtime.acquireLock(paths)
    await removeStaleSupervisorSocket(paths)
    const token = await runtime.loadToken(paths)
    const publishEvent = async (event: SupervisorEvent): Promise<void> => {
    ...
    const validateHost: ListenForBridgesOptions['validateHost'] = hello => {
      let accepted = false
      const admission = helloTail.then(async () => {
        if (hello.launch.profile !== arguments_.profile) return
        const previous = activeLaunch
        if (previous !== undefined) {
          if (!sameLaunchIdentity(previous, hello.launch)) return
          if (
            previous.pid !== hello.hostPid
    ...
    const bridgeOptions: ListenForBridgesOptions = {
      ...
      onCommand: async (command, peer) => {
        ...
        switch (command.type) {
          case 'get-status':
            ...
            return { ok: true }
          case 'update-config':
            if (command.config.enabled) await supervisor!.updateConfig(command.config)
            else await supervisor!.pause()
            return { ok: true }
          case 'rebuild':
...
async function runSupervisorHandoff(
  arguments_: Extract<SupervisorCliArguments, { mode: 'handoff' }>,
  runtime: SupervisorCliRuntime,
): Promise<void> {
  const paths = await runtime.resolvePaths(arguments_.profile)
  ...
  let committedLaunch: HostLaunchSpec | undefined
  let committedConfig: SupervisorConfig | undefined
  let standbyLease: LockLease | undefined
  const follow = createHandoffFollow({
    ...
    releaseOwnership: async () => {
      ...
      if (standbyLease === undefined) return
    ...
    beginServing: async () => {
      ...
      await serveSupervisor(
        { mode: 'serve', profile: arguments_.profile },
        runtime,
        {
...
export async function runLeadHandoff(options: {
  ...
  const lockHooks: HandoffLockHooks = {
    ...
    reacquire: async () => {
      if (getLease() !== undefined) return true
  ...
  let handle: HandoffChannelHandle | undefined
  const deadline = Date.now() + 15_000
  for (;;) {
    try {
  ...
  let handoff: ReturnType<DevReloaderSupervisor['handoff']> | undefined
  ...
  try {
    ...
    failures.push(error)
    if (handoff?.phase === 'committed') shouldShutdown = true
  }

  if (handoff?.phase === 'committed') shouldShutdown = true
  if (shouldShutdown) onCommitted()
...
export function createSupervisorCliAdapters(
  runtime: SupervisorCliRuntime = defaultRuntime(),
): SupervisorCliAdapters {
  return {
...
export async function runSupervisorCli(
  ...
  adapters: SupervisorCliAdapters = createSupervisorCliAdapters(),
): Promise<void> {
