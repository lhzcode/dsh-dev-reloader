...
interface PendingWatchEvent {
  readonly root: string
  readonly event: RawWatchEvent
}

interface ActivePlan {
  readonly plan: WatchPlan
  readonly ignored: (path: string) => boolean
  readonly pendingEvents: Map<string, PendingWatchEvent>
  readonly sessions: Map<string, WatchBackendSession>
  readonly degradedRoots: Map<string, DegradedWatchRoot>
  readonly closingSessions: Set<Promise<void>>
  discarded: boolean
}
...
function pathStyle(path: string): 'posix' | 'windows' {
...
function normalizeAbsolute(path: string): string {
  if (pathStyle(path) === 'windows') return win32.resolve(path)
  return resolve(path)
}

function relativeWithin(root: string, candidate: string): string | undefined {
  const style = pathStyle(root)
  if (pathStyle(candidate) !== style) return undefined
...
function equivalentPathKey(path: string): string {
  const normalized = normalizeAbsolute(path)
  return pathStyle(normalized) === 'windows' ? normalized.toLowerCase() : normalized
}

function watchRoots(projects: readonly ProjectDescriptor[]): string[] {
  const byEquivalentPath = new Map<string, string>()
  for (const rawRoot of projects.flatMap(project => [project.root, project.workspaceRoot])) {
    ...
    if (current === undefined || root < current) byEquivalentPath.set(key, root)
  }
  const unique = [...byEquivalentPath.values()].sort()

  return unique.filter(candidate =>
    !unique.some(root =>
      equivalentPathKey(root) !== equivalentPathKey(candidate)
      && isPathWithin(root, candidate),
    ),
  )
}

function isDefaultIgnored(path: string): boolean {
...
function createIgnoredPredicate(plan: WatchPlan, roots: readonly string[]): (path: string) => boolean {
  ...
  return rawPath => {
    ...
    if (isDefaultIgnored(absolutePath)) return true
    if (outputRoots.some(root => isPathWithin(root, absolutePath))) return true

    const absolutePosix = toPosix(absolutePath)
    if (matchers.some(matches => matches(absolutePosix))) return true
...
const chokidarBackend: WatchBackend = {
  start(roots, ignored, onEvent, onError, signal) {
    ...
    return new Promise((resolveSession, rejectSession) => {
      ...
      watcher.on('add', path => {
        if (!closed) onEvent({ kind: 'add', absolutePath: path })
      })
      watcher.on('change', path => {
        if (!closed) onEvent({ kind: 'change', absolutePath: path })
      })
      watcher.on('unlink', path => {
        if (!closed) onEvent({ kind: 'unlink', absolutePath: path })
      })
      watcher.once('ready', () => {
        if (settled) return
        settled = true
...
function normalizeSetupTimeout(value: number | undefined): number {
  if (value === undefined) return DEFAULT_WATCH_SETUP_TIMEOUT_MS
...
function watchSetupTimeout(root: string, timeoutMs: number): Error {
...
export function createWatchPlanController(
  options: CreateWatchPlanControllerOptions,
): WatchPlanController {
  ...
  const statusOf = (plan: ActivePlan | undefined, promoted: boolean): WatchPlanStatus => ({
    promoted,
  ...
  const deliverEvent = (plan: ActivePlan, event: RawWatchEvent): void => {
    try {
  ...
  const reportDegraded = (candidate: ActivePlan, degraded: DegradedWatchRoot): void => {
    if (candidate.discarded || candidate.degradedRoots.has(degraded.root)) return
  ...
  const closeSession = (session: WatchBackendSession): Promise<void> => {
    try {
  ...
  const closeRootSession = (plan: ActivePlan, root: string): void => {
  ...
  const closeSessions = async (plan: ActivePlan): Promise<void> => {
    plan.discarded = true
  ...
  const settleSetup = (
    root: string,
    started: Promise<WatchBackendSession>,
    controller: AbortController,
  ): Promise<
    | { readonly root: string; readonly session: WatchBackendSession }
    | { readonly root: string; readonly error: Error }
  > => new Promise(resolveSetup => {
    let settled = false
    ...
    const settle = (
      ...
      if (settled) {
        if ('session' in result) void closeSession(result.session).catch(notifyError)
        return
      }
      settled = true
  ...
  const replace = (plan: WatchPlan): Promise<WatchPlanStatus> => {
    if (closed) return Promise.reject(new Error('watch plan controller is closed'))

    const roots = watchRoots(plan.projects)
    const candidate: ActivePlan = {
    ...
    const setups = roots.map(root => {
      ...
      let started: Promise<WatchBackendSession>
      try {
        started = backend.start(
          [root],
          candidate.ignored,
          rawEvent => {
            if (closed || candidate.discarded || candidate.degradedRoots.has(root)) return
            if (active === candidate) deliverEvent(candidate, rawEvent)
            else candidate.pendingEvents.set(rawEvent.absolutePath, { root, event: rawEvent })
          },
          error => {
            if (closed || candidate.discarded) return
    ...
    const operation = promotions.then(async (): Promise<WatchPlanStatus> => {
      ...
      if (closed) {
        ...
        const aborted = results.find(result =>
          'error' in result && result.error.name === 'AbortError')
        if (aborted !== undefined && 'error' in aborted) throw aborted.error
  ...
  const inspect = (): WatchPlanStatus => statusOf(active, active !== undefined)

  const close = (): Promise<void> => {
    if (closePromise !== undefined) return closePromise
    closed = true
    for (const controller of pendingSetups) controller.abort()
    closePromise = promotions.then(async () => {
      const previous = active
      active = undefined
      if (previous !== undefined) await closeSessions(previous)
    })
    return closePromise
  }
